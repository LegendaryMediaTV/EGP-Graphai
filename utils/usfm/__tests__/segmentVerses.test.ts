...
): { text: string; paragraph?: boolean; break?: boolean }[] {
  return blocks.map(({ text, paragraph, break: brk }) => ({
    text,
    paragraph,
    break: brk,
  }));
...
describe("segmentVerses — Genesis 1-2 (a chapter boundary mid-token-stream, a footnote aside excluded)", () => {
  ...
  it("should emit exactly 31 verses for chapter 1 and 25 for chapter 2, none dropped or duplicated", () => {
    const chapterOne = records
      .filter((record) => record.chapter === 1)
      .map((record) => record.verse);
    const chapterTwo = records
      .filter((record) => record.chapter === 2)
      .map((record) => record.verse);
    expect(chapterOne).toEqual(
      Array.from({ length: 31 }, (_, index) => index + 1),
    );
    expect(chapterTwo).toEqual(
  ...
  it("should keep verse 1's own Strong's-tagged words as plain text, with the H8064 footnote body excluded entirely", () => {
    const verseOne = records.find(
      (record) => record.chapter === 1 && record.verse === 1,
    );
    expect(verseOne?.rawContent).toBe(
  ...
  it("should exclude a footnote naming God's Hebrew name from verse 4 of chapter 2, keeping the surrounding prose intact", () => {
    const verseFour = records.find(
      (record) => record.chapter === 2 && record.verse === 4,
    );
    expect(verseFour?.rawContent).not.toContain("Yahweh” is God’s proper Name");
    expect(verseFour?.rawContent).toContain(
...
describe("segmentVerses — Luke 17:35-37 (verse 36 is entirely a textual-variant footnote, no verse text of its own)", () => {
  ...
  it("should not apply the fallback to verse 37, which carries real verse text of its own", () => {
    ...
    expect(verse37?.rawContent).toBe(
...
describe("segmentVerses — Genesis 1 (\\p opens a prose paragraph on the block that follows, at chapter start and mid-chapter alike)", () => {
  ...
  it("should carry no flag on verse 2's own block — no \\p between verses 1 and 2 in the real source", () => {
    const verseTwo = records.find(
      (record) => record.chapter === 1 && record.verse === 2,
    );
  ...
  it("should flag verse 3's own first block paragraph: true — a real mid-chapter \\p, not just a chapter-start special case", () => {
    const verseThree = records.find(
      (record) => record.chapter === 1 && record.verse === 3,
    );
...
describe("segmentVerses — Numbers 13:1-5 (\\p and \\m both open a paragraph on the block that follows; \\m splits a verse mid-stream and also crosses a verse boundary)", () => {
  ...
  it("should flag verse 1's only block paragraph: true (\\p before it)", () => {
    const verseOne = records.find((record) => record.verse === 1);
    expect(blockFlags(verseOne?.blocks ?? [])).toEqual([
      { text: "Yahweh spoke to Moses, saying,", paragraph: true },
    ]);
  });
  ...
  it("should split verse 4 into two blocks at the mid-verse \\m, the first unflagged and the second paragraph: true", () => {
    ...
    expect(blockFlags(verseFour?.blocks ?? [])).toEqual([
      ...
      {
        text: "Of the tribe of Reuben, Shammua the son of Zaccur.",
        paragraph: true,
      },
    ]);
  });
...
describe('segmentVerses — the real \\b-then-bare-\\qN idiom itself (Genesis 49:2→3: \\q2 ...father. / \\b / \\q1 / \\v 3 "Reuben...), isolated from the two-block-boundary assertions above — a naive fix that dispatched \\b as flushBlock(true) would fail this exact case', () => {
  ...
  it("should still let verse 3's own first block open paragraph: true — the bare \\q1 being absorbed rather than dispatched doesn't cost the pending paragraph flag \\b itself already set", () => {
    const verseThree = records.find((record) => record.verse === 3);
    expect(blockFlags(verseThree?.blocks ?? [])[0]).toMatchObject({
      paragraph: true,
    });
  });
});
...
describe("segmentVerses — Job 5:26-6:2 (a real \\b stanza break sitting directly on a chapter boundary — Job 5's own last verse into chapter 6's own first)", () => {
  ...
  it("should flag Job 6:1's own first (and only) block paragraph: true — pendingParagraph survives the \\c 6 boundary itself, the same way it already survives an ordinary \\p/\\m boundary — reinforced here by chapter 6's own real \\p, which would have set the identical flag on its own", () => {
    const chapterSixVerseOne = records.find(
      (record) => record.chapter === 6 && record.verse === 1,
    );
...
describe("segmentVerses — Job 16:22-17:1 (the real chapter-boundary shape of the \\b-then-bare-\\qN idiom: \\b \\c 17 \\q1 \\v 1... — the suppression guard must survive \\c itself, since \\c is neither whitespace nor a break marker and would otherwise let the bare \\q1 behind it resurrect the break \\b just dropped)", () => {
  const records = segmentVerses(
    readFixture("job-16-22-17-1-chapter-b.usfm"),
    "JOB",
  );
  ...
  it("should flag chapter 17 verse 1's own first line paragraph: true, alongside its own ordinary break: true", () => {
    const chapterSeventeenVerseOne = records.find(
      (record) => record.chapter === 17 && record.verse === 1,
    );
...
describe("segmentVerses — Psalm 46:11-47:1 (the heading-adjacent shape of the same idiom: \\b \\c 47 \\d ... \\q1 \\v 1... — the guard must also survive a \\d/\\sp/\\s1/\\qc heading marker, not just \\c, since a Psalm superscription routinely sits between a chapter-ending \\b and its own bare \\qN)", () => {
  const records = segmentVerses(
    readFixture("psalm-46-11-47-1-heading-b.usfm"),
    "PSA",
  );
  ...
  it("should still attach the \\d superscription as Psalm 47:1's own leading heading block, ahead of its own real first line, which itself carries paragraph: true and break: true", () => {
    const chapterFortySevenVerseOne = records.find(
      (record) => record.chapter === 47 && record.verse === 1,
    );
    const blocks = chapterFortySevenVerseOne?.blocks ?? [];
    expect(blocks[0]?.headingContent).toMatchObject({
      subtitle: "For the Chief Musician. A Psalm by the sons of Korah.",
    });
...
describe("segmentVerses — Deuteronomy 31:28-32:2 (a \\b-less chapter boundary — \\p \\v 30 ... \\c 32 \\q1 \\v 1..., no \\b anywhere near it — gets the identical clean-cut, chapter-paragraph-start convention as a \\b-adjacent chapter boundary)", () => {
  const records = segmentVerses(
    readFixture("deuteronomy-31-28-32-2.usfm"),
    "DEU",
  );
  ...
  it("should flag chapter 32 verse 1's own first line paragraph: true, even though nothing but a bare \\q1 sits directly behind \\c 32", () => {
    const chapterThirtyTwoVerseOne = records.find(
      (record) => record.chapter === 32 && record.verse === 1,
    );
...
describe("segmentVerses — Psalm 90:16-91:2 (the same \\b-less chapter-boundary convention, with no \\d superscription in the gap at all — Psalm 91 carries none)", () => {
  ...
  it("should flag Psalm 91:1's own first line paragraph: true", () => {
    const chapterNinetyOneVerseOne = records.find(
      (record) => record.chapter === 91 && record.verse === 1,
    );
...
describe("segmentVerses — Psalm 41:11-42:2 (the \\ms1-adjacent shape: \\c 42 \\ms1 BOOK 2 \\d ... \\q1 \\v 1..., no \\b at all — the guard must survive \\ms1 too, not only \\c and the heading markers, or the bare \\q1 sitting behind all three still reaches back across the boundary)", () => {
  const records = segmentVerses(
    readFixture("psalm-41-11-42-2-ms1-d.usfm"),
    "PSA",
  );
  ...
  it("should still attach the \\d superscription as Psalm 42:1's own leading heading block, ahead of its own real first line, which itself carries paragraph: true — with the real \\ms1 book-division heading (blocks[0]) landing ahead of both, the identical stacking order the isolated psalm-42-opening.usfm fixture already establishes", () => {
    const chapterFortyTwoVerseOne = records.find(
      (record) => record.chapter === 42 && record.verse === 1,
    );
...
describe('segmentVerses — Psalm 1 into Psalm 2 (this chapter boundary gets the same clean-cut, chapter-paragraph-start convention as every other one, not a cross-chapter "same rule, two sides" reach)', () => {
  ...
  it("should leave Psalm 1:6's own last block clean, with no break: true reach-back from Psalm 2's own opening \\q1 — matching WEBUS2020's own real upstream HEAD", () => {
    const blocks =
      records.find((record) => record.chapter === 1 && record.verse === 6)
        ?.blocks ?? [];
    expect(last(blockFlags(blocks))).toEqual({
  ...
  it("should flag Psalm 2:1's own block paragraph: true — the chapter boundary's own clean cut, not a reach-back that never fires", () => {
    const psalm2VerseOne = records.find(
      (record) => record.chapter === 2 && record.verse === 1,
    );
...
describe("segmentVerses — Genesis 1:1 (Strong's attachment: the leading-space convention and the forward-merge default, verified against real dense \\w tagging)", () => {
  ...
  it('should attach the untagged connector comma to the trailing edge of "beginning", the strong-carrying node it ends, per convention #3 (tight punctuation trails backward, never leads forward like an ordinary connector word) — matching utils/auditNodes.ts\'s own "leading punctuation glued to the wrong node" check', () => {
    ...
    expect(nodes.find((node) => node.strong === "H7225")).toEqual({
      text: " beginning,",
      strong: "H7225",
    });
    expect(
      nodes.find((node) => node.strong === "H8064" && node.text === " God"),
    ).toBeDefined();
  });
});
...
describe("segmentVerses — Genesis 1:1, the includeStrongs toggle (utils/importUsfm.ts's ImportOptions.strongs)", () => {
  ...
  it("should leave output byte-identical to today's default when includeStrongs is true or omitted", () => {
    ...
    const withExplicitTrue = segmentVerses(
      readFixture("genesis-1-2.usfm"),
      "GEN",
      undefined,
      true,
    ...
    const verseOne = withDefault.find(
      (record) => record.chapter === 1 && record.verse === 1,
    );
    expect(verseOne?.blocks[0].nodes).toEqual([
      { text: "In", strong: "H8064" },
...
describe("segmentVerses — John 14:16 (\\wj Words of Christ, a nested \\+w Strong's word inside it, a bare untagged connector, and a footnote splitting one \\wj utterance into two source spans)", () => {
  ...
  it('should tag every word in both \\wj spans marks: ["woc"], including the \\+w-nested Strong\'s words, which also keep their own strong number', () => {
    ...
    expect(nodes.find((node) => node.text === " will")).toMatchObject({
      strong: "G1510",
      marks: ["woc"],
    });
  });
  ...
  it('should still reunite the two \\wj spans into one continuous marks: ["woc"] run once the footnote\'s own space folds onto the next span\'s first word — the footnote now carries real content instead of being dropped, but the reunion mechanism is unchanged: the bare joining space between \\f* and the reopened \\wj has no marks of its own, so it folds forward onto "that" rather than reopening a gap', () => {
    const verseSixteen = records.find((record) => record.verse === 16);
    const nodes = verseSixteen?.blocks[0]?.nodes ?? [];
    expect(nodes.find((node) => node.text === " that")).toMatchObject({
      strong: "G2443",
      marks: ["woc"],
    });
    expect(nodes.every((node) => node.marks?.includes("woc"))).toBe(true);
...
describe("segmentVerses — Psalm 119:1-16 (the \\d ALEPH/\\d BETH acrostic boundary mid-stream)", () => {
  const records = segmentVerses(
    readFixture("psalm-119-aleph-beth.usfm"),
    "PSA",
  );
  ...
  it("should exclude both \\d ALEPH and \\d BETH from every verse's own content", () => {
    expect(records.some((record) => record.rawContent.includes("ALEPH"))).toBe(
      false,
    );
    expect(records.some((record) => record.rawContent.includes("BETH"))).toBe(
      false,
    );
  });
...
describe("segmentVerses — Acts 7:37 (a footnote and a cross-reference back to back, no text between them — both must survive, neither overwriting the other)", () => {
  ...
  it("should attach both the \\f's own trn footnote and the \\x's own xrf cross-reference, on two separate nodes, since one ContentObject can carry only one foot (the real WEBUS2020 shape that first exposed this: \"brothers, like me.\" is followed immediately by \\f...\\f*\\x...\\x* with nothing between them)", () => {
    ...
    const nodes =
      verseThirtySeven?.blocks.flatMap((block) => block.nodes ?? []) ?? [];
    const footed = nodes.filter((node) => node.foot !== undefined);
    expect(footed).toHaveLength(2);
    expect(footed[0].foot).toEqual({
      type: "var",
      content: "TR adds \u201cYou shall listen to him.\u201d",
    });
    expect(footed[1]).toEqual({
      foot: { type: "xrf", content: { bibleLink: "Deuteronomy 18:15" } },
    });
  });
});
...
describe("segmentVerses — 2 Kings 12:1-5 (a mid-verse cross-reference, attached to the connector text its own \\x marker immediately follows)", () => {
  ...
  const records = segmentVerses(
    readFixture("2-kings-12-1-5.usfm"),
    "2KG",
    canon,
  );
  ...
  it('should attach a real xrf footnote, resolved to Exodus 30:12, to the connector text "evaluated," the \\x marker immediately follows in the real source', () => {
    ...
    expect(footed?.foot).toEqual({
      type: "xrf",
      content: { bibleLink: "Exodus 30:12" },
    });
...
describe("segmentVerses — Psalm 3's own \\d superscription lands as a real subtitle on verse 1, before the paragraph content", () => {
  ...
  it("should still flag verse 1's own real content blocks break: true, unaffected by the subtitle now sitting ahead of them", () => {
    ...
    expect(
      verseOne?.blocks
        .slice(1)
        .map((block) => ({ text: block.text, break: block.break })),
    ).toEqual([
...
describe('segmentVerses — Psalm 119\'s acrostic \\d markers, including the two real \\w-tagging artifacts on "HE" and "SIN AND SHIN"', () => {
  it("should attach ALEPH as verse 1's own first block and BETH as verse 9's, with verse 8's own last real block still carrying break: true unaffected", () => {
    ...
    expect(verseOne?.blocks[0]).toEqual({
      text: "",
      headingContent: { heading: "ALEPH", type: "acrostic" },
    });
    expect(last(verseEight?.blocks ?? [])).toMatchObject({
      text: "Don’t utterly forsake me.",
      break: true,
    });
    expect(
      verseEight?.blocks.some((block) => block.headingContent !== undefined),
    ).toBe(false);
    expect(verseNine?.blocks[0]).toEqual({
      text: "",
      headingContent: { heading: "BETH", type: "acrostic" },
    });
  });
  ...
  it('should strip the stray \\w tag from "HE" and still classify it as an acrostic heading, not an ordinary subtitle', () => {
    ...
    expect(verseThirtyThree?.blocks[0]).toEqual({
      text: "",
      headingContent: { heading: "HE", type: "acrostic" },
    });
  });
...
describe("segmentVerses — Psalm 1's own \\ms1 BOOK 1 (no \\d superscription on this psalm) and Psalm 42's own BOOK 2 + \\d combination", () => {
  it("should attach the first book-division heading to Psalm 1:1, ahead of its own paragraph content, with the range computed from this run's own emitted chapter data", () => {
    ...
    expect(verseOne?.blocks[0]).toEqual({
      text: "",
      headingContent: {
        heading: [{ text: "Book One", marks: ["sc"] }, " (Psalms 1–1)"],
      },
    });
  ...
  it("should take the ordinal word from the label's own printed numeral, so this fixture's opening BOOK 2 says \"Book Two\" even though it is the first division in the file", () => {
    ...
    const kinds = verseOne?.blocks.map((block) =>
      block.headingContent === undefined ? "content" : "heading",
    );
...
describe("segmentVerses — Song of Solomon's \\sp speaker labels, both across a verse boundary and mid-verse", () => {
  const records = segmentVerses(
    readFixture("song-of-solomon-1-1-5.usfm"),
    "SNG",
  );
  ...
  it("should attach \"Beloved\" to verse 2's own first block, never to verse 1's own trailing block, even though the \\sp marker sits in verse 1's own token stream before the verse boundary (another real source's Song of Solomon 1:2 precedent shape)", () => {
    ...
    const verseTwo = records.find((record) => record.verse === 2);
    expect(
      verseOne?.blocks.some((block) => block.headingContent !== undefined),
    ).toBe(false);
    expect(verseTwo?.blocks[0]).toEqual({
      text: "",
      headingContent: { heading: "Beloved" },
    });
  });
  ...
  it('should insert "Friends" and "Beloved" as their own mid-verse blocks inside verse 4, in source order, between the real text blocks that surround them', () => {
    ...
    const kinds = verseFour?.blocks.map((block) =>
      block.headingContent !== undefined
        ? (block.headingContent as { heading?: unknown }).heading
        : block.text,
    );
  ...
  it("should open a paragraph on the block after each \\sp, including the two mid-verse ones inside verse 4, and on no other block", () => {
    ...
    expect(blockFlags(verseFour?.blocks ?? [])).toEqual([
      ...
      {
        text: "The king has brought me into his rooms.",
        paragraph: undefined,
        break: true,
      },
      { text: "", paragraph: undefined, break: undefined },
      {
        text: "We will be glad and rejoice in you.",
        paragraph: true,
        break: true,
      },
      {
  ...
  it("should leave every verse with no \\sp of its own unflagged, so the paragraph tracks the speaker change and nothing else", () => {
    for (const verse of [3, 5]) {
      const blocks =
        records.find((record) => record.verse === verse)?.blocks ?? [];
...
describe("segmentVerses — Numbers 21:14's \\bk/\\bk* book-title citation (tagged marks: [\"i\"] on each of its own real Strong's-tagged words, not dropped as plain text)", () => {
  it("should tag every one of the citation's own 7 \\+w-tagged words marks: [\"i\"] individually — each keeps its own Strong's number, since a marks mismatch against its plain-text neighbors on both sides stops any of them from coalescing into the surrounding text", () => {
    ...
    expect(nodes[citationStart + marked.length]).toMatchObject({
      text: ", “Vaheb in",
    });
  });
});
...
describe("segmentVerses — Ezra 4's own \\mi/\\pi1 letter-quoting markers, flattened to paragraph like \\p/\\m/\\nb", () => {
  ...
  it('should flag the block \\mi opens ("To King Artaxerxes...") paragraph: true', () => {
    const verseEleven = records.find((record) => record.verse === 11);
    expect(last(blockFlags(verseEleven?.blocks ?? []))).toMatchObject({
      paragraph: true,
    });
  });
  ...
  it("should flag verse 12's own first block paragraph: true from the \\pi1 immediately before it", () => {
    const verseTwelve = records.find((record) => record.verse === 12);
    expect(blockFlags(verseTwelve?.blocks ?? [])[0]).toMatchObject({
      paragraph: true,
    });
  });
});
...
describe("segmentVerses — \\s1 (Baruch 6, a chapter-start pericope heading with nothing accumulated yet)", () => {
  it("should attach \\s1's own text as verse 1's own first block, ahead of the paragraph content — the identical chapter-start mechanism already proven for a Psalm superscription", () => {
    ...
    expect(verseOne?.blocks[0]).toEqual({
      text: "",
      headingContent: { heading: "The Letter of Jeremy (Jeremiah)" },
    });
...
describe("segmentVerses — \\s1 (Daniel 3:90/91, mid-chapter, no embedded footnote of its own)", () => {
  it("should insert the heading between verse 90 and verse 91, verse 91's own separate footnote landing on its own content, never on the heading", () => {
    ...
    expect(
      verseNinety?.blocks.some((block) => block.headingContent !== undefined),
    ).toBe(false);
    expect(verseNinetyOne?.blocks[0]).toEqual({
      text: "",
      headingContent: { heading: "Deliverance from the Furnace" },
    });
...
describe("segmentVerses — \\s1 (Daniel 13 and 14, chapter-start pericope headings — plain text, each book's own real chapter-1-of-the-pericope footnote landing on the verse content that follows, never on the heading)", () => {
  it('should attach "THE HISTORY OF SUSANNA" to Daniel 13\'s own verse 1', () => {
    const records = segmentVerses(
      readFixture("daniel-13-s1-opening.usfm"),
      "DAG",
    );
    const chapterThirteenVerseOne = records.find(
      (record) => record.chapter === 13 && record.verse === 1,
    );
    expect(chapterThirteenVerseOne?.blocks[0]).toEqual({
      text: "",
      headingContent: { heading: "THE HISTORY OF SUSANNA" },
    });
    expect(chapterThirteenVerseOne?.blocks[1]).toMatchObject({
      paragraph: true,
    });
  });
  ...
  it("should tag all 4 of the verse-1 footnote's own real \\+bk citations marks: [\"i\"] on the verse content's own footnote, not the heading (Daniel 13:1)", () => {
    ...
    expect(foot?.content).toContainEqual({
      text: "The History of Susanna",
      marks: ["i"],
    });
  ...
  it('should attach "Bel and the Dragon" to Daniel 14\'s own verse 1', () => {
    const records = segmentVerses(
      readFixture("daniel-14-s1-opening.usfm"),
      "DAG",
    );
    const chapterFourteenVerseOne = records.find(
      (record) => record.chapter === 14 && record.verse === 1,
    );
    expect(chapterFourteenVerseOne?.blocks[0]).toEqual({
      text: "",
      headingContent: { heading: "Bel and the Dragon" },
    });
    expect(chapterFourteenVerseOne?.blocks[1]).toMatchObject({
      paragraph: true,
    });
  });
...
describe("segmentVerses — \\pc (2 Maccabees 1:18-19, a decorative divider sandwiched between two real \\b stanza breaks, mid-verse-boundary, with real text on both sides)", () => {
  it("should drop the dash-divider's own text entirely, leaking into neither verse 18 nor verse 19, and should drop break: true from verse 18's own last line entirely (the first \\b) rather than keep it — the second \\b (after the dropped \\pc text) is a harmless no-op, since nothing accumulated for it to close, and \\p already guarantees verse 19's own paragraph: true independent of either \\b", () => {
    const records = segmentVerses(
      readFixture("2-maccabees-1-16-19-pc.usfm"),
      "2MA",
    );
...
describe("segmentVerses — \\cp/\\d (Psalm 151's own front matter — \\cp sits before any \\v, already structurally harmless; \\d's own embedded footnote lands the same way a Psalm superscription's already does)", () => {
  it("should drop \\cp's own text entirely and attach the \\d superscription (with its embedded footnote) as verse 1's own first real heading block", () => {
    ...
    const verseOne = records.find((record) => record.verse === 1);
    const headingBlocks =
      verseOne?.blocks.filter((block) => block.headingContent !== undefined) ??
      [];

    expect(
      verseOne?.blocks.every((block) => !(block.text ?? "").includes("151")),
    ).toBe(true);
    const subtitleBlock = headingBlocks.find(
      (block) =>
        block.headingContent !== undefined &&
        "subtitle" in (block.headingContent as object),
    );
...
describe("segmentVerses — \\ip (every \\ip block becomes a footnote on a textless leading node, attached to the book's own verse 1:1)", () => {
  ...
  it('should keep both of an \\ip block\'s own two embedded \\bk citations, each tagged marks: ["i"], and classify the real, Septuagint-naming body var, not every \\ip is stu (Baruch)', () => {
    ...
    expect(introNode?.foot?.content).toContainEqual({
      text: "The Letter of Jeremiah",
      marks: ["i"],
    });
    expect(introNode?.foot?.content).toContainEqual({
      text: "Baruch",
  ...
  it("should attach Esther-Greek's own two \\ip blocks as two separate textless leading nodes, in source order, not one merged block — \\is1 contributing nothing, and never confused with the real, ordinary footnote-leading content block Esther-Greek 1:1's own real double-\\fl note independently produces", () => {
    ...
    expect(introBlocks[1].nodes?.[0].foot?.content).toContain(
      "KJV versification",
    );
  ...
  it("should attach Sirach's own two \\ip blocks — the modern editorial blurb and the real, ancient Prologue alike — as two separate textless leading nodes in source order, with nothing lost to chrome for either one, ahead of the real poetry content that follows", () => {
    ...
    expect(verseOne?.blocks[0]).toBe(introBlocks[0]);
    expect(verseOne?.blocks[1]).toBe(introBlocks[1]);
    expect(verseOne?.blocks[2]).toMatchObject({
...
describe("segmentVerses — already-established mechanisms need no change for the deuterocanon corpus (Daniel 4's own real \\pi1/\\q3 fixture)", () => {
  it("should flag every \\pi1-opened block paragraph: true, including the one carrying across the verse 1→2 boundary, exactly as Ezra's own \\pi1 already does", () => {
    ...
    expect(blockFlags(verseOne?.blocks ?? [])).toEqual([
      ...
      {
        text: "to all the peoples, nations, and languages, who dwell in all the earth:",
        paragraph: true,
      },
      { text: "Peace be multiplied to you.", paragraph: true },
    ]);
    expect(blockFlags(verseTwo?.blocks ?? [])[0]).toMatchObject({
      paragraph: true,
    });
  });
  ...
  it("should flag every \\q3-ended block break: true exactly like \\q1/\\q2 already do — this corpus's own first real \\q3 exercise (canonical scope is 100% \\q3-free)", () => {
    ...
    expect(blockFlags(verseThree?.blocks ?? [])).toEqual([
      ...
      {
        text: "His dominion is from generation to generation.",
        break: undefined,
      },
    ]);
  });
...
describe("segmentVerses — ASV1901's real \\qc (Psalm 119's acrostic letter heading, on a different marker than \\d)", () => {
  it('should attach a real {heading: [...], type: "acrostic"} block for \\qc\'s own "א ALEPH." ahead of verse 1\'s own paragraph content, splitting the Hebrew letter from its transliterated name', () => {
    const records = segmentVerses(
      readFixture("asv1901-psalm-119-aleph.usfm"),
      "PSA",
    );
    const verseOne = records.find((record) => record.verse === 1);
    expect(verseOne?.blocks[0]).toEqual({
...
describe("segmentVerses — ASV1901's real \\qc, the mid-corpus case (Psalm 119:7-9, the BETH transition)", () => {
  const records = segmentVerses(
    readFixture("asv1901-psalm-119-beth.usfm"),
    "PSA",
  );
  ...
  it("should leave verse 8's own last block ending cleanly at \"utterly.\" with break: true, never carrying BETH's own heading text", () => {
    const verseEight = records.find((record) => record.verse === 8);
    expect(last(verseEight?.blocks ?? [])).toMatchObject({
      text: "Oh forsake me not utterly.",
      break: true,
    });
...
describe("segmentVerses — ASV1901's real \\add (translator-supplied words), a construct WEB's own corpus never carries", () => {
  it('should tag "and" with marks: ["i"] and no strong, as its own node, from Genesis 1:11\'s real \\add span — left/right neighbors (", " and the connector-merged "fruit-trees bearing") stay split from it, the same marks-mismatch rule that already keeps \\qs\'s own Selah split from its neighbors', () => {
    const records = segmentVerses(
      readFixture("asv1901-genesis-1-11.usfm"),
      "GEN",
    );
    const verseEleven = records.find((record) => record.verse === 11);
    const nodes = verseEleven?.blocks[0]?.nodes ?? [];
    const addIndex = nodes.findIndex(
      (node) => "marks" in node && node.marks?.includes("i"),
    );
    expect(nodes[addIndex]).toEqual({ text: "and", marks: ["i"] });
    expect(nodes[addIndex - 1]).toEqual({ text: ", " });
    expect(nodes[addIndex + 1]).toMatchObject({
      text: " fruit-trees bearing",
      strong: "H6213",
    });
  });
  ...
  it('should tag "the" with marks: ["i"] and no strong from 2 Peter 1:1\'s real \\add span, sitting directly between two \\w-tagged words with no plain prose to lean on — the joining space on each side folds onto its own leading edge as usual, but the marks mismatch still keeps it split from both "and" and "Saviour"', () => {
    const records = segmentVerses(
      readFixture("asv1901-2peter-1-1.usfm"),
      "2PE",
    );
    const verseOne = records.find((record) => record.verse === 1);
    const nodes = verseOne?.blocks[0]?.nodes ?? [];
...
describe('segmentVerses — MSB2025\'s real "LORD" (Genesis 2:4): no marks: ["sc"] is ever synthesized from plain source text', () => {
  it('should render "LORD" as plain text with its own strong number and no marks at all — a source that spells a whole phrase in full capitals, with nothing in its own markup to signal a size change, is shouting in caps, not asking for small-caps styling. MSB2025 carries zero \\nd/\\sc markup anywhere in its own source, so this needs no code change: `InlineMarkName` (`"woc" | "i"`) has no "sc" member at all, and segmentVerses/inlineMarks.ts never synthesizes one from source content', () => {
    const records = segmentVerses(
      readFixture("msb2025-genesis-2-4-lord.usfm"),
      "GEN",
    );
    const verseFour = records.find((record) => record.verse === 4);
    const nodes = verseFour?.blocks.flatMap((block) => block.nodes ?? []) ?? [];
...
describe("segmentVerses — MSB2025's real Genesis 1:1-5: \\m immediately before every single \\v produces paragraph: true on every verse's own first block, the existing marker-type rule applied to a corpus where \\m is not rare but exclusive", () => {
  const records = segmentVerses(
    readFixture("msb2025-genesis-1-1-5.usfm"),
    "GEN",
  );
...
describe("segmentVerses — ASV1901's real Job 4:8-12 poetry (\\q1 lines, a real \\b stanza break, then \\q2): the two-part \\b rule produces correct output for this source's own real poetry density too, not just WEB's own", () => {
  ...
  it("should flag verse 12's own first line paragraph: true from the \\b, in addition to its own ordinary break: true from the trailing \\q1 that follows it (not from the bare \\q2 right after \\b, already absorbed rather than spent as a reach-back), and leave its second, fixture-final line unflagged since nothing follows it here", () => {
    ...
    expect(blockFlags(verseTwelve?.blocks ?? [])).toEqual([
      {
        text: "Now a thing was secretly brought to me,",
        paragraph: true,
        break: true,
      },
      {
        text: "And mine ear received a whisper thereof.",
        paragraph: undefined,
        break: undefined,
      },
    ]);
...
describe("segmentVerses — fraction normalization reaches ordinary verse text too (synthetic — no real WEBUS2020 instance outside a footnote)", () => {
  ...
  it("should normalize a genuine ASCII fraction in ordinary verse prose, not only inside a footnote", () => {
    const verseOne = records.find((record) => record.verse === 1);
    expect(verseOne?.rawContent).toBe(
      `It was about ${uniformFraction("2", "3")} of a bushel.`,
    );
    expect(verseOne?.blocks[0]?.text).toBe(
      `It was about ${uniformFraction("2", "3")} of a bushel.`,
    );
  });
});
...
describe("segmentVerses — fraction normalization reaches the empty-verse fallback too (synthetic — mirrors the real Luke 17:36 footnote-only shape above, with a fraction added)", () => {
  const records = segmentVerses(
    "\\v 1 \\f + \\fr 1:1 \\ft It was about 2/3 of a bushel.\\f*",
    "GEN",
  );
...
describe("segmentVerses — ASV1901's real \\ms1 BOOK I (Psalm 1:1, the Roman-numeral spelling of the division label)", () => {
  const records = segmentVerses(
    readFixture("asv1901-psalm-1-1-ms1-book-i.usfm"),
    "PSA",
  );
...
describe("segmentVerses — ASV1901's \\ms1 BOOK II written ahead of its own \\c 42 (Psalm 41:1-42:2, 20-PSAeng-asv.usfm)", () => {
  const records = segmentVerses(
    readFixture("asv1901-psalm-41-42-2-ms1-book-ii.usfm"),
    "PSA",
  );
  ...
  it("should leave Psalm 41:1 carrying only its own \\d superscription, never the division heading the chapter-counter reading would have attached there", () => {
    const chapterFortyOneVerseOne = records.find(
      (record) => record.chapter === 41 && record.verse === 1,
    );
  ...
  it("should leave Psalm 41:13's own last block clean, since a division written between two chapters is not content of the one it closes", () => {
    const chapterFortyOneVerseThirteen = records.find(
      (record) => record.chapter === 41 && record.verse === 13,
    );
    expect(
      last(blockFlags(chapterFortyOneVerseThirteen?.blocks ?? [])),
    ).toEqual({ text: "Amen, and Amen." });
...
describe("segmentVerses — ASV1901's \\ms1 BOOK IV ahead of \\c 90 (Psalm 90:1, 20-PSAeng-asv.usfm)", () => {
  const records = segmentVerses(
    readFixture("asv1901-psalm-90-opening-ms1-book-iv.usfm"),
    "PSA",
  );
  ...
  it('should say "Book Four" from the Roman numeral the label prints, and open on Psalm 90, even though this is the only division the fixture carries', () => {
    ...
    expect(verseOne?.blocks[0]).toEqual({
      text: "",
      headingContent: {
        heading: [{ text: "Book Four", marks: ["sc"] }, " (Psalms 90–90)"],
      },
    });
  });
});
...
describe("segmentVerses — \\ms, \\ms2 and \\ms3, the major-section levels no source on disk carries (synthetic)", () => {
  ...
  it.each(["ms", "ms1", "ms2", "ms3"])(
    ...
    (marker) => {
      const verseTwo = segmentedWith(marker).find(
        (record) => record.verse === 2,
      );
      expect(verseTwo?.blocks[0]).toEqual({
        text: "",
        headingContent: { heading: "The Book of Immanuel" },
      });
    },
  );
  ...
  it.each(["ms", "ms2", "ms3"])(
    ...
    (marker) => {
      const verseOne = segmentedWith(marker).find(
        (record) => record.verse === 1,
      );
...
describe("segmentVerses — \\mr, the reference range under a major-section heading (synthetic)", () => {
  ...
  it("should hang the range on its own heading's text node as an xrf footnote, with the display parentheses dropped and the target resolved to a bibleLink", () => {
    ...
    expect(verseTwo?.blocks[0]).toEqual({
      ...
      headingContent: {
        heading: {
          text: "The Reign of Solomon",
          foot: { type: "xrf", content: { bibleLink: "1 Kings 1–11" } },
        },
      },
    });
  });
  ...
  it("should throw rather than guess when an \\mr follows no major-section heading, since there is then no heading text to hang it on", () => {
    ...
    ).toThrow(/follows no major-section heading/);
  });
});
...
describe("segmentVerses — an \\ms1 outside Psalms (synthetic — no source on disk carries \\ms1 anywhere but Psalms)", () => {
  ...
  it("should open the content after the heading with paragraph: true, the same promise \\d/\\sp/\\s1 already make", () => {
    ...
    expect(blockFlags(verseOne?.blocks.slice(1) ?? [])).toEqual([
      {
        text: "It happened in the days of Ahaz.",
        paragraph: true,
        break: undefined,
      },
    ]);
...
describe("segmentVerses — an \\ms1 inside Psalms that is not a book-division label (synthetic)", () => {
  ...
  it("should treat it as an ordinary heading on its own printed text, since being in Psalms is not by itself a book division", () => {
    ...
    expect(verseOne?.blocks[0]).toEqual({
      text: "",
      headingContent: { heading: "The Songs of Ascent" },
    });
    expect(JSON.stringify(records)).not.toContain("(Psalms");
  });
});
...
describe("segmentVerses — ASV1901's real \\qc ח HHETH. (Psalm 119:56-57), a spelling no standard transliteration table carries", () => {
  const records = segmentVerses(
    readFixture("asv1901-psalm-119-hheth.usfm"),
    "PSA",
  );
...
describe("segmentVerses — a \\qc carrying a real centered poetic line (synthetic — every \\qc on disk is a letter heading)", () => {
  ...
  it("should end the line before it with break: true, exactly as \\q1/\\q2/\\q3 do", () => {
    ...
    expect(blockFlags(verseOne?.blocks ?? [])).toEqual([
      ...
      {
        text: "Extol him, all you peoples!",
        paragraph: undefined,
        break: true,
      },
    ]);
...
describe("segmentVerses — a poetic \\qc immediately after a chapter boundary (synthetic — the clean-cut guard must absorb it the way it absorbs a bare \\qN)", () => {
  ...
  it("should leave chapter 1's own last block clean, with no break: true reaching back across the boundary", () => {
    ...
    expect(blockFlags(last(chapterOne)?.blocks ?? [])).toEqual([
      {
        text: "The first psalm's last line.",
        paragraph: true,
        break: undefined,
      },
    ]);
