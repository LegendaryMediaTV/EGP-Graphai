...
function makeTempVersionDir(): string {
  return fs.mkdtempSync(
    path.join(os.tmpdir(), "overhaul-embedded-references-"),
  );
}
...
describe("computeReferenceOverhaul — preview, read-only", () => {
  it("should find and report a reference sitting unlinked inside an ordinary footnote's own prose, and leave the file untouched on disk", () => {
        ...
        content: [
          {
            ...
            foot: {
              type: "stu",
              content: "This is quoted as a messianic prophecy in Isaiah 9:6.",
            },
          },
        ],
      },
    ...
    const before = fs.readFileSync(
      path.join(versionDir, "01-GEN.json"),
      "utf-8",
    );
    ...
    expect(fs.readFileSync(path.join(versionDir, "01-GEN.json"), "utf-8")).toBe(
      before,
    );
  });
    ...
    writeBookFixture(versionDir, "01-GEN.json", [
      {
        ...
        content: [
          {
            text: "the earth",
            foot: { type: "stu", content: "A cubit is about 18 inches." },
          },
        ],
      },
    ]);
    ...
    writeBookFixture(versionDir, "01-GEN.json", [
      {
        ...
        verse: 1,
        content: [
          { text: "God", foot: { type: "xrf", content: "Isaiah 9:6" } },
        ],
      },
    ]);
  ...
  it("should reach a footnote nested in a verse's content, subtitle, and heading alike, matching overhaulFootnotes.ts's own reclassifyFootnotesIn descent", () => {
        ...
        content: [
          {
            heading: [
              {
                text: "Superscription",
                foot: { type: "stu", content: "See also Isaiah 9:6." },
              },
            ],
          },
          {
            subtitle: [
              {
                text: "A prayer.",
                foot: { type: "stu", content: "Compare Matthew 1:23." },
              },
            ],
          },
          {
            ...
            foot: {
              type: "stu",
              content: "In John 3:16 a similar phrase appears.",
            },
          },
        ],
      },
    ...
    expect(changes.map((c) => c.target)).toEqual(
      expect.arrayContaining(["Isaiah 9:6", "Matthew 1:23", "John 3:16"]),
    );
  });
        ...
        content: [
          {
            ...
            foot: {
              type: "stu",
              content: "See especially I Kgs. 8:33 for the fuller context.",
            },
          },
        ],
      },
        ...
        content: [
          {
            ...
            foot: {
              type: "stu",
              content: "See also Isaiah 9:6; Matthew 1:23.",
            },
          },
        ],
      },
    ...
    writeBookFixture(versionDir, "01-GEN.json", [
      {
        ...
        content: [
          {
            text: "God",
            foot: { type: "stu", content: "Compare 1 Maccabees 3:38." },
          },
        ],
      },
    ]);
    ...
    expect(computeReferenceOverhaul(versionDir).changes).toEqual([
      {
        ...
        chapter: 1,
        verse: 1,
        raw: "1 Maccabees 3:38",
        target: "1 Maccabees 3:38",
      },
    ]);
  });
    ...
    writeBookFixture(versionDir, "01-GEN.json", [
      {
        ...
        content: [
          {
            text: "he will crush",
            foot: { type: "stu", content: "See Isaiah 9:6." },
          },
        ],
      },
    ]);
    writeBookFixture(versionDir, "40-MAT.json", [
      {
        ...
        content: [
          {
            text: "Immanuel",
            foot: { type: "stu", content: "Compare Isaiah 7:14." },
          },
        ],
      },
    ]);
...
describe("applyReferenceOverhaul — --fix, writes changed books only", () => {
  it("should write back only the book whose footnotes actually changed, leaving an unchanged sibling book byte-identical", async () => {
    ...
    const psalmBefore = fs.readFileSync(
      path.join(versionDir, "19-PSA.json"),
      "utf-8",
    );

    await applyReferenceOverhaul(versionDir);

    const genesisAfter = JSON.parse(
      fs.readFileSync(path.join(versionDir, "01-GEN.json"), "utf-8"),
    );
    expect(genesisAfter[0].content[0].foot.content).toEqual([
      "See ",
      { bibleLink: "Isaiah 9:6" },
      ".",
    ]);
    expect(fs.readFileSync(path.join(versionDir, "19-PSA.json"), "utf-8")).toBe(
      psalmBefore,
    );
  });
    ...
    expect(result.changes).toEqual([
      {
        ...
        verse: 11,
        raw: "18:6ff",
        target: "2 Samuel 18:6",
      },
    ]);
    const written = JSON.parse(
      fs.readFileSync(path.join(versionDir, "10-2SM.json"), "utf-8"),
    );
    expect(written[0].content[0].foot.content).toEqual([
      "Amnon’s scandal with Tamar (",
      { bibleLink: "2 Samuel 13:14", content: "13:14" },
...
describe("parseOverhaulArgs — version required, book id an optional second positional", () => {
  ...
  it("should parse a bare version with fix defaulting to false and no book", () => {
    expect(parseOverhaulArgs(["WEBUS2020"])).toEqual({
      fix: false,
  ...
  it("should parse a version alongside --fix in either order", () => {
    expect(parseOverhaulArgs(["WEBUS2020", "--fix"])).toEqual({
      fix: true,
      versionArg: "WEBUS2020",
      bookArg: undefined,
    });
    expect(parseOverhaulArgs(["--fix", "WEBUS2020"])).toEqual({
      fix: true,
      versionArg: "WEBUS2020",
      bookArg: undefined,
    });
  ...
  it("should parse a version and a book id together, with or without --fix", () => {
    expect(parseOverhaulArgs(["WEBUS2020", "GEN"])).toEqual({
      fix: false,
      versionArg: "WEBUS2020",
      bookArg: "GEN",
    });
    expect(parseOverhaulArgs(["WEBUS2020", "GEN", "--fix"])).toEqual({
      fix: true,
      versionArg: "WEBUS2020",
      bookArg: "GEN",
    });
...
describe("findSwallowedFlags — npm eats a flag unless a bare -- precedes it", () => {
  it("should report a flag npm consumed instead of forwarding", () => {
    expect(findSwallowedFlags(["ASV1901"], { npm_config_fix: "true" })).toEqual(
      ["--fix"],
    );
  });
  ...
  it("should report nothing when the flag actually arrived", () => {
    expect(
      findSwallowedFlags(["ASV1901", "--fix"], { npm_config_fix: "true" }),
    ).toEqual([]);
  });
