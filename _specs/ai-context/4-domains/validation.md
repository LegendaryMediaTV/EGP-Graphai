...
The Validation domain ensures data integrity across all Bible JSON files, and `npm run validate` is the sole entry point for every normalization and validation rule this repo enforces on `bible-versions/**` — see "One Entry Point" below. Its own auto-fix pass normalizes key ordering, JSON formatting, `bibleLink` dashes and ranges, fractions, ellipses, and straight-quote direction; tags an untagged non-Latin script run; repairs several Strong's-node placement conventions, including where a footnote marker's own joining space belongs; drops a meaningless empty-text remnant and a duplicate footnote anchor; unlinks a `bibleLink` target the version cannot resolve; merges adjacent siblings that differ in nothing but text; and rolls a stray mark-boundary whitespace node onto whichever neighbor is the smaller mark set — then proves that whole pass is a fixed point of itself before checking anything else. It then validates schemas, book ordering, file naming, verse structure, and cross-references between entities, and finally runs report-only audits for the same version(s): declared chapter counts, cross-chapter links, truncated ranges, Strong's-node placement, and unresolvable `bibleLink` targets. Runs as a pre-commit check and CI gate.

**`npm run validate` is expected to exit clean.** There is no version, and no finding, this repo treats as a standing exception — a red run always names a real gap to close. See [bible-versions.md](../4-domains/bible-versions.md) for how a version whose source content is still incomplete (e.g. missing deuterocanonical chapters) stays green: its `_version.json` declares only what its verse files actually carry, and the declared count moves up in the same change that imports the rest.
...
The auto-fix pass runs first, in this order, then a fixed-point check confirming that pass has nothing left to change on a second application, then the hierarchical checks, then the report-only audits. See "One Entry Point" below for why this is the only way any of it runs.
...
7. **Ellipsis Normalization** – Rewrites an un-normalized dot run to U+2026, this repo's ellipsis convention; deliberately leaves a bare two-period run untouched as a standing rule (see [strongs-node-audit.md](../4-domains/strongs-node-audit.md), the ellipsis check)
8. **Straight-Quote Direction Normalization** – Resolves each ASCII `'`/`"` to its correctly directed curly counterpart (the straight-quote check): opens after whitespace, an opening bracket, or the start of a string, and closes otherwise; an adjacent already-resolved quote character inherits that neighbor's own direction, so a bunched-up run (`"'"`) nests correctly; see [strongs-node-audit.md](../4-domains/strongs-node-audit.md), the straight-quote check
9. **Greek Dialytika Repair** – Composes a dialytika written after its accent, or left uncomposed, into the letter it belongs to (the dialytika check). Confined to the affected letter rather than normalizing the whole string to NFC, which would fold the Greek ano teleia (U+0387) and question mark (U+037E) that this corpus uses in thousands of places; see [strongs-node-audit.md](../4-domains/strongs-node-audit.md), the dialytika check
10. **Untagged Script-Run Tagging** – Splits a node's own text at a Hebrew or Greek letter run embedded in otherwise-Latin prose into its own `{text, script}` node (the script-run check); declines and reports when the node also carries `strong`, `foot`, `marks`, or any property beyond bare text, since splitting it would have to decide which fragment keeps the property; see [strongs-node-audit.md](../4-domains/strongs-node-audit.md), the script-run check
11. **Unmerged Connector Merge** – Folds an untagged connector forward into the `strong`/`foot`/`break`-carrying neighbor immediately after it (the unmerged-connector check); see [strongs-node-audit.md](../4-domains/strongs-node-audit.md)
12. **Leading-Punctuation Reattach** – Moves a leading run of tight punctuation off a node that cannot merge forward — one carrying `strong`, `foot`, or `break` — onto the node before it, where the punctuation already belonged (the leading-punctuation check). A node that turns out to have been nothing but that punctuation hands its `foot` to the target, when the target carries none, and disappears; where the target is already footnoted, it stays behind as a bare `{foot}` sibling rendering its marker in the same place. Its `break` moves only when the two nodes were already adjacent: where the check reached back across a textless Strong’s sibling, the break stays put on a bare `{break: true}` node, since a line break crossing that sibling would move its Strong’s tag to the far side of the break. Rendered text never changes; a footnote marker only ever moves later, to the far side of punctuation it should already have followed; see [strongs-node-audit.md](../4-domains/strongs-node-audit.md), the leading-punctuation check
13. **Footnote Punctuation Reorder** – Moves tight closing punctuation ahead of a footnote marker that would otherwise render before it, when the move is safe (the footnote-punctuation-order check); declines and leaves a real finding in place otherwise
14. **Mark-Boundary Embedded-Space Relocation** – Relocates whitespace embedded in a marked node's own leading/trailing text onto the matching edge of a same-marked neighbor, when the move is safe (the mark-boundary-embedded-space check); declines and leaves a real finding in place otherwise
15. **Footnote-Marker Spacing Relocation** – Moves a joining space off the trailing edge of a `foot`-carrying node onto the leading edge of the node after it — the same leading-space convention the trailing-whitespace check already enforces for `strong`, extended here to `foot` (the footnote-marker-spacing check). Asks the render-order question, not just "does this node's own text end in whitespace," so a textless `{foot}` anchor is caught too; declines and leaves a real finding in place when there is no real next node at this array level, a `break`/`paragraph` boundary sits at the join, the next node's own text already starts with whitespace, or the two sides disagree in `marks`/`script`; see [strongs-node-audit.md](../4-domains/strongs-node-audit.md), the footnote-marker-spacing check
16. **Empty Text Key Drop** – Drops an empty `text: ""` key from a node that carries something else alongside it (real KJV1769 shape: `{text: "", foot: {...}}`), leaving every other property untouched. A node whose *only* property is an empty `text`, or that carries no properties at all, is a different question this step doesn't answer — see the Meaningless Content Nodes check below
17. **Duplicate Footnote Anchor Removal** – Deletes a node that renders no visible text of its own whose `foot` is byte-for-byte identical to the nearest surviving node before it (the duplicate-footnote-anchor check). The far more common shape — one note correctly annotating two real, separate word occurrences, each on its own text-bearing node — is never touched, since the later node there still renders text of its own; see [strongs-node-audit.md](../4-domains/strongs-node-audit.md), the duplicate-footnote-anchor check
18. **Unresolvable bibleLink Unlink** – Strips the `bibleLink` wrapper off a target that parses but names a book, chapter, or verse the version does not carry, keeping its display text (or the bare target string when there is no override) as plain content; declines when an override is present but renders no visible text; never touches a target the endpoint grammar cannot parse at all — that is a different, deliberately excluded case; see [cross-chapter-links.md](../4-domains/cross-chapter-links.md)
19. **Mergeable Sibling Merge** – Normalizes a `{text}`-only object into a bare string, then folds a maximal run of adjacent siblings that agree in `marks`/`script` into one node (the mergeable-sibling check). A node carrying `strong`, `foot`, `bibleLink`, nested `content`, `paragraph`, or `break` is never eligible on either side; see [strongs-node-audit.md](../4-domains/strongs-node-audit.md), the mergeable-sibling check
20. **Mark-Boundary Space Merge** – Rolls a bare, untagged whitespace-only sibling node onto whichever real neighbor is the smaller mark set, covering both exact `marks`/`script` agreement and a subset match (the mark-boundary-space check); runs after the mergeable-sibling check, since that step's own merge can leave a bare space newly adjacent to a node it just folded siblings around, and before the heading-paragraph check. Unlike the other gated steps in this pass, it never declines with a real finding left behind — the one shape it leaves untouched (a smaller side that already carries `strong`/`foot`) is already correct, not a residual problem; see [strongs-node-audit.md](../4-domains/strongs-node-audit.md), the mark-boundary-space check
21. **Heading Paragraph Flag** – Adds a missing `paragraph: true` to the node right after a heading/subtitle run (the heading-paragraph check), flat and corpus-wide, with no per-book judgment
22. **Auto-Fix Pass Fixed-Point Check** – Re-applies the content-rewriting steps above, in the same order, to the in-memory content of every file the pass just changed, and fails by name — file, verse, and step — if any of them would still report a change on this second application. On a settled corpus nothing changed, so nothing is even re-read; the moment two steps interact, this catches it in the run that introduced the interaction rather than requiring a second, manual `npm run validate` to notice
23. **Schema Validation** – JSON conforms to JSON Schema Draft-07
24. **Book Order Integrity** – Orders start at 1, sequential, no gaps or duplicates
25. **File Existence** – Expected verse files exist for each book in version
26. **File Naming** – Files match `{order}-{bookId}.json` pattern
27. **Book Field Match** – Verse `book` field matches filename book ID
28. **Reference Integrity** – Book IDs in versions exist in books registry
29. **Meaningless Content Nodes** – Flags a node that renders nothing: `marks`/`script` with no `text` to apply them to (a non-greedy bold/italic delimiter pairing can't match zero characters and leaks into surrounding text), or an empty `text: ""` riding alongside anything else the node still carries — not only when `text` is the node's *sole* key, but also a husk like `{text: "", foot: {...}}` that keeps a stray `foot`/`break` after its own marks were stripped. `foot`, `strong`, `morph`, `lemma`, `bibleLink`, and bare `paragraph`/`break` are meaningful with **no `text` key at all**, and none of those combinations is ever flagged; the gap this check closes is specifically an empty *string*, never an absent key
30. **Strong's Trailing Whitespace** – Flags a `strong`-carrying node whose own `text` ends in whitespace, violating the convention that a joining space belongs on the *following* node's leading edge, not the tagged node's trailing edge
31. **Declared Chapter Count Audit** (report-only) – Compares a book's declared `chapters` count in `_version.json` against the highest chapter its own verse file actually carries, in both directions. A version whose source content is still incomplete declares only what it actually has — see [bible-versions.md](../4-domains/bible-versions.md) for how that stays true across an import rather than producing a standing finding
32. **Cross-Chapter Link Audit** (report-only) – Delegates to `findCrossChapterLinks()` for each version being validated; see [cross-chapter-links.md](../4-domains/cross-chapter-links.md)
33. **Truncated bibleLink Range Audit** (report-only) – Delegates to `findTruncatedRanges()` for each version being validated, its own labeled count distinct from the Cross-Chapter Link Audit above; see [cross-chapter-links.md](../4-domains/cross-chapter-links.md)
34. **Strong's-Node Placement Audit** (report-only) – Delegates to `auditVersion()` for each version being validated, covering every check that domain owns; see [strongs-node-audit.md](../4-domains/strongs-node-audit.md)
35. **Unresolvable bibleLink Target Audit** (report-only) – Delegates to `findUnresolvableTargets()` for each version being validated, reporting whatever the Unresolvable bibleLink Unlink step above declined to unlink; see [cross-chapter-links.md](../4-domains/cross-chapter-links.md)
36. **Abbreviation Reference Audit** (report-only) – Delegates to `findUnknownAbbreviations()` for each version being validated, reporting an `{ abbr }` id its own version's registry does not define and an `_id` that registry defines twice. No auto-fix: an unresolved id is either a typo in the content or a missing registry entry, and only a person can say which. Registries are per-version with no fallback, so there is nowhere for a failed lookup to go; see [bible-versions.md](../4-domains/bible-versions.md)
...
3. `bibleLink` – Scriptural reference target
4. `abbr` – Abbreviation reference into the version's registry
5. `paragraph` – Paragraph marker (boolean or object)
6. `type` – Footnote type
7. `text` – Text content
8. `content` – Nested content array
9. `script` – Script indicator (H/G)
10. `marks` – Formatting marks (alphabetized)
11. `break` – Line break indicator
12. `foot` – Footnote reference
13. `strong` – Strong's number
14. `morph` – Morphological code
15. `lemma` – Lexical lemma
...
- **The trailing audits are peers, not a pipeline** – The hierarchical checks above them each assume the earlier ones held, so a failure exits immediately. The declared-chapter-count audit, the cross-chapter link audit, the truncated-range audit, the Strong's-node audit, and the unresolvable-`bibleLink`-target audit depend on neither each other nor anything upstream, so every one of them always runs to completion and reports in full before `main()` exits non-zero. A version that fails one still gets audited by the others in the same run, so a real regression in any of them is never masked by an early exit on a different finding.
...
- **Corpus completeness is checked at three separate grains, not assumed from a green schema.** File existence and naming (the File Existence and File Naming checks) confirm every book a version declares has exactly one file; book ordering (the Book Order Integrity check) confirms no gap or duplicate in the numbering; and the declared chapter count audit (the Declared Chapter Count Audit) confirms each book's own `_version.json` entry agrees with the chapters its file actually carries. A version can pass every schema check and still be missing real content — CLV1880's own Esther and Daniel are exactly that today.
...
- **"Report what it can't fix" is not a separate code path — it falls out of the pass ordering.** Every gated step among truncated-range reconstruction (step 4), untagged script-run tagging (step 10), footnote punctuation reorder (step 13), mark-boundary embedded-space relocation (step 14), footnote-marker spacing relocation (step 15), and the unresolvable-`bibleLink` unlink (step 18) either fixes a finding or declines it with a named reason. Whatever it declines is still sitting on disk by the time the report-only audits run after the pass, so the identical finding surfaces there with actionable detail — with no extra code written to make that connection happen.
...
```typescript
// Every loop below runs to completion regardless of the others' outcome.
// Unlike the hierarchical exits above, none of these audits depends on
// any other, so a version failing one still gets checked by the rest
// in the same run.
