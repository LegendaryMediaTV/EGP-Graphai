...
- **Symbolic apparatus notation is `var`, checked right after `xrf`.** A Greek or Hebrew critical edition prints its apparatus as operators between competing readings rather than as the prose every vocabulary rule looks for, so `⇒` (one reading against another), a standalone `~` (the compared edition omits the verse), and `¦` (one witness group's reading against the next) each mark a variant on their own. Without this construct all 10,228 of BYZ2026's `¦` bodies fall through to `stu` (the `⇒` and standalone `~` operators came from the since-retired BYZ2018, which BYZ2026 does not use). A leading Greek or Hebrew character is deliberately *not* a fourth signal: it says which edition a note came from rather than what the note is, so it misreads a translation gloss that opens with the original-language word it glosses.
...
- **The `xrf` citation-only grammar recognizes several further real shapes, not just a plain chapter:verse.** A sub-verse letter (`23:29–30a`), one edition's own three-token Song of Solomon abbreviation (`"S of Sol 8:12"`, the one exception to the citation grammar's normal one-word book-name cap), and Obadiah cited bare with no chapter or verse at all (the corpus's only real bare-book-name citation, so it's scoped to that one book rather than generalized to every single-chapter book) all still count as "nothing but citations." A `"Fulfilled in ..."`/`"Foretold in ..."` lead-in is stripped before that check runs, so a real `"Fulfilled in II Chron 29:8"` is judged on its citation content alone rather than falling through to `stu` on the leftover verb.
...
- **A named-book embedded mention no longer requires a verse to link — a real rule reversal, not just a widening.** `EMBEDDED_HEAD` now matches chapter-only (Genesis 3:24's real "See Ezekiel 10." links the whole chapter; so do the self-referential "Psalm 34 is an acrostic poem..." notes at Psalm 34:1/111:1/112:1), where the prior `EMBEDDED_REFERENCE_SUFFIX` it replaced required an explicit verse. A bare *ambient* parenthetical citation with no book name of its own (`AMBIENT_HEAD`, the `"(12)"`-shaped case) deliberately keeps the verse-mandatory rule instead: with no named book anchoring it, a bare chapter number floating in prose reads too easily as an unrelated footnote index or list item.
- **A comma-separated list after a chapter-only head reads as more chapters, never that chapter's own verses.** `findSafeReferenceLength()` only walks a comma-list extension when the head it just matched named a verse; a chapter-only head (a real MSB2025 Maskil note citing `"Psalms 32, 42, 44–45, 52–55, 74, 78, 88–89, and 142"`, thirteen psalms) used to have its trailing numbers read as impossible verses of the first chapter named. A dash range still extends a chapter-only head either way, since `"Genesis 4–9"` names one continuous span rather than a list.
- **The citation-only test now resolves a book name against the registry, not a one-word cap alone.** `REGISTRY_BOOK_NAME` in `footnoteTypeRules.ts` adds every registry spelling `ONE_WORD_BOOK`'s twelve-letter, single-word cap can't express — a full `"1 Thessalonians 5:1–11"`, a spelled-out `"Song of Solomon"` — so a citation-only footnote naming one of those no longer falls through to `stu` for not fitting the one-word slot. The same book-name slot is barred from ever matching a printed-edition or manuscript siglon (`LXX`, `TR`, `WH`, and others): a real MSB2025 Acts 27:37 variant note, `"WH 76"` (the number 276), used to read as chapter 76 of an invented book called `WH`.
...
- **The acrostic letter-name table now spans many editions' own spellings, and combined stanza headings.** `ACROSTIC_LETTER_NAMES` in `headings.ts` used to hold only WEB's own 22 transliterations (misspelling "KAPF" included, kept so already-imported data still classifies); it now also carries every variant a shipped or attested edition uses for the same Hebrew letter (KJV1769's "Bet", CLV1880's "deleth", ASV1901's own "HHETH" misspelling, and others), and `isAcrosticLetterName` splits on a joiner (`AND`, `&`, `/`, a dash) and requires every resulting part to be a letter name, so a source's own combined two-letter stanza heading ("SIN AND SHIN", "Sin/Shin", "He – Vav") is recognized as one construct instead of needing its own table entry per joiner style per pair.
- **A Psalter book-division heading is recognized by its own printed text, not tied to one specific marker.** `psalterBookDivisionNumber()` accepts any of `\ms`/`\ms1`/`\ms2`/`\ms3` carrying a `"BOOK n"`/`"BOOK N"` (Arabic or Roman numeral) label, rather than assuming `\ms1` alone; a marker in that family that isn't a division label keeps its own printed text like any other major-section heading. Before this, a division heading arriving on any marker other than `\ms1` reached the generic heading branch with `skipToNextMarker` left false, so its text was silently swept into the surrounding verse's own prose instead of becoming a heading — a real gap this closes, not a theoretical one. `isAcrosticGlyphHeading()` gained the same kind of content-based check for `\qc`: earlier versions of this pipeline assumed every `\qc` in the corpus was Psalm 119's acrostic glyph heading, but an edition can use `\qc` for USFM's ordinary centered poetic line too, so segmentation now peeks at a `\qc` span's own text before deciding which builder it belongs to.
- **The non-Latin glyph range behind acrostic-glyph detection and `splitScriptRuns.ts` needed a presentation-form block it was missing.** Written as "everything that isn't Latin, punctuation, a digit, or whitespace" rather than named Unicode block ranges, so no fixed list of blocks has to already be complete — and one was short by exactly the Hebrew presentation-form shin a shipped acrostic heading prints, until that block was added to the shared range both `headings.ts` and `splitScriptRuns.ts` read from.
...
Psalm 119's 22 stanza markers share the plain `\d` marker with every ordinary Psalm superscription; this lookup, not marker choice or position, is what tells them apart. Splitting on a joiner before checking membership is what lets a single combined-stanza heading naming two letters ("SIN AND SHIN") pass the same test a single-letter heading does.
