...
- **`splitScriptRuns` moved from a missing external scaffold to a tracked module.** `utils/usfm/headings.ts`, `footnotes.ts`, and `verify.ts` all import it from `./splitScriptRuns` (`utils/usfm/splitScriptRuns.ts`), not from the gitignored `imports/_lib/` path earlier versions of this pipeline assumed. The dependency that remains gitignored is the WEBUS2020 raw USFM corpus at `imports/webus2020/ebible-usfm/`, which `metadata.test.ts`, `verify.test.ts`, and the upstream-convention specs read directly; it's present in this checkout but, being gitignored, isn't guaranteed to exist in a fresh one. Running the full suite (`npx vitest run`) currently reports all 29 test files passing; see [6-tests-and-build.md](../6-tests-and-build.md) for what a checkout without that corpus would still be missing.
