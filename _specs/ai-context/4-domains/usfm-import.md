...
- **The acrostic letter-name table now spans many editions' own spellings, and combined stanza headings.** `ACROSTIC_LETTER_NAMES` in `headings.ts` used to hold only WEB's own 22 transliterations (misspelling "KAPF" included, kept so already-imported data still classifies); it now also carries every variant a shipped or attested edition uses for the same Hebrew letter (KJV1769/NET2019's "Bet", CLV1880's "deleth", ASV1901's own "HHETH" misspelling, and others), and `isAcrosticLetterName` splits on a joiner (`AND`, `&`, `/`, a dash) and requires every resulting part to be a letter name, so a source's own combined two-letter stanza heading ("SIN AND SHIN", "Sin/Shin", "He – Vav") is recognized as one construct instead of needing its own table entry per joiner style per pair.
