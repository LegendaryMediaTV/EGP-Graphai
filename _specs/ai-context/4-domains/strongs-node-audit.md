...
- **Fix check 1's findings** – `npx ts-node utils/fixUnmergedNodes.ts YLT1898 --fix` (preview without `--fix`); see [utils/fixUnmergedNodes.ts](../../../utils/fixUnmergedNodes.ts)
- **Fix check 6's findings** – `npx ts-node utils/fixHeadingParagraphs.ts WEBUS2020 --fix` (preview without `--fix`); see [utils/fixHeadingParagraphs.ts](../../../utils/fixHeadingParagraphs.ts)

## Key Business Rules

- **This file only detects; two checks now have a separate, safe fixer.** `auditNodes.ts` itself ships no `--fix` flag and never writes. Checks 1 and 6 are flat enough that a mechanical fixer can't get the direction wrong, so [utils/fixUnmergedNodes.ts](../../../utils/fixUnmergedNodes.ts) and [utils/fixHeadingParagraphs.ts](../../../utils/fixHeadingParagraphs.ts) exist for exactly those two — each imports this file's own eligibility functions (`canJoinForward`, `findHeadingParagraphMismatches`) rather than re-deriving the judgment, so there's exactly one "is this safe" decision, not two copies that could drift apart. The remaining five checks stay manual: the "which direction does this word belong" call they make explicit is exactly the kind of judgment a mechanical fixer would get wrong on real Bible text.
