...
- **Validation** – `npm run validate` is the one command that runs every normalization and validation rule this repo enforces: a sixteen-step auto-fix pass (key order, JSON formatting, `bibleLink` dash/range normalization and unresolvable-target unlinking, fraction/ellipsis normalization, script-run tagging, and several Strong's-node placement repairs), checked against itself for a fixed point, then schema/structure checks, then five report-only audits (declared chapter counts, cross-chapter links, truncated `bibleLink` ranges, Strong's-node placement, unresolvable `bibleLink` targets). No separate audit script or `--fix` flag exists anywhere else in the tree
- **Strong's-Node Placement Audit** – Detects sixteen ways a node's text-flow placement can drift from this repo's own conventions; eight of the sixteen repair themselves inside the auto-fix pass above
...
## Recent Changes (Audit/Fix Tooling Consolidated into `npm run validate`)
...
- **Report-Only Audits Run as Peers, Not a Pipeline** – The five trailing audits (declared chapter counts, cross-chapter links, truncated ranges, Strong's-node placement, unresolvable `bibleLink` targets) depend on none of each other, so all five always run to completion and report in full even after one fails — needed because CLV1880's own declared-chapter-count finding is now a permanent, accepted part of every run
- **CLV1880's Two Standing Findings Are Accepted, Not a Bug** – `npm run validate` reports exactly two findings for this version, permanently, until its Esther and Daniel deuterocanonical content is imported. `_version.json` still declares the full chapter counts on purpose; shrinking them to match would conceal the gap rather than close it
...
8. **Leading-Space Convention** – A joining space belongs on the leading edge of the node it joins, never the trailing edge of the node before it, and never as a verse's own opening character. Applies to every attribute that can own that boundary, not just the first one that got a check: enforced for `strong` and, as of check 12, for `foot` too. `auditNodes.ts` detects every violation; `npm run validate`'s own auto-fix pass repairs the `strong` and `foot` cases automatically
...
✅ **1,265 tests passing, across all 38 test files** (Vitest):

- `functions/__tests__/` (10 files): `contentSchema.test.ts` (4), `convertToSmallCaps.test.ts` (40), `getBibleVersions.test.ts` (23), `mapContentText.test.ts` (14), `mergeEquivalentSiblingsInContent.test.ts` (18), `normalizeEllipses.test.ts` (19), `normalizeFractions.test.ts` (24), `sortContentKeys.test.ts` (27), `tagScriptRunsInContent.test.ts` (19), `writeJsonFile.test.ts` (10)
- `utils/__tests__/` (12 files): `auditNodes.test.ts` (173, the largest suite in the repo), `crossChapterLinks.test.ts` (89), `exportContent.test.ts` (115), `fixDuplicateFootnoteAnchors.test.ts` (8), `fixFootnoteMarkerSpacing.test.ts` (23), `fixFootnotePunctuationOrder.test.ts` (5), `fixHeadingParagraphs.test.ts` (4), `fixMarkBoundaryEmbeddedSpaces.test.ts` (5), `fixUnmergedNodes.test.ts` (5), `importUsfm.test.ts` (19), `overhaulFootnotes.test.ts` (22), `validate.test.ts` (76)
- `utils/usfm/__tests__/` (15 files, the USFM import pipeline): `segmentVerses.test.ts` (117), `verify.test.ts` (98), `footnoteTypeRules.test.ts` (93), `footnotes.test.ts` (56), `references.test.ts` (37), `inlineMarks.test.ts` (28), `headings.test.ts` (15), `metadata.test.ts` (14), `embeddedReferenceConventions.test.ts` (14), `blockStructure.test.ts` (14), `tokenize.test.ts` (10), `paragraphNoise.test.ts` (7), `bibleLinkTargetConventions.test.ts` (6), `chapterBoundaryUpstreamConvention.test.ts` (4), `bMarkerUpstreamConvention.test.ts` (3)
- `web/public/js/__tests__/footnoteText.test.ts` – 7 tests for shared footnote-text extraction

All 38 files load and pass in this checkout, which carries the gitignored WEBUS2020 raw USFM corpus several `utils/usfm/__tests__/` specs read directly; see [6-tests-and-build.md](6-tests-and-build.md#usfm-import-pipeline-domain) for which ones and what a checkout without it would still be missing.
