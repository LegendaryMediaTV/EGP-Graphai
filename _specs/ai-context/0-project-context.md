...
- **Lexical Map** – Per-language codex under `lexical-maps/` keyed by dictionary root, listing every attested inflected form with its parse as category-tagged codes. Index numbers such as Strong's attach to the parse, so one word can hold several numbers and one number cover several words without conflict. A corpus tagged only at the lemma level is refined against it. No validator reads the directory yet
...
## Recent Changes (Lexical Map Added, BYZ2026 Strong's Numbers Refined From It)

- **Lexical Map Format** – New `lexical-maps/` tree with four file kinds and a schema for each: a language registry (`_language.json`) naming inflection categories, codes, letters and the transliteration table; one morphology file per morph code system holding its positional grammar and token map; one codex file per letter of the alphabet keyed by dictionary root; and one index file per external index holding cell-level placement rules. Greek is populated. The design decision behind all of it is that a concordance index cannot double as a lexical inventory, so the root is the key and the number is a value on a parse. See [4-domains/lexical-map.md](4-domains/lexical-map.md) and [documentation/EGP-Graphai/lexical-map.md](../documentation/EGP-Graphai/lexical-map.md)
- **BYZ2026 Retagged From the Map** – The Byzantine text was tagged at the lexical level, so every form of εἰμί carried G1510 and the numbers Strong's assigns to the individual principal parts went unused. Matching each token's spelling and morphology to a cell replaced the lemma-level number with the one the index actually assigns that form, across a large share of the edition's tagged tokens. `exports/text-vbv-strongs/BYZ2026/` carries the numbers verbatim and was regenerated
- **Placement Rules Corrected and Extended** – The crosswalk was checked token by token against a Strong's-tagged Greek text and against the index's own occurrence count per number, which caught rules that a KJV-derived check alone had got wrong: a gender split Strong's does not make, comparatives assigned an adverb's number, two spellings of one noun flattened onto one entry, and spelling rules that missed the positional-accent twin of an enclitic. Rules for ὁράω's principal parts were added, matching what already existed for εἰμί and λέγω. Where an index splits one spelling and parse by *sense*, no rule can express it: the cell now lists every number involved, the corpus tag decides each token, and the number is recorded under `unplaced` with the reason
- **Four Misfiled Forms Relemmatized** – The adverbial εὐθύς sat under εὐθέως, a different word; ἐκπερισσοῦ, ἐάνπερ and ἔνθεν sat under the roots they merely resemble. Fused compounds written solid get their own root here, elisions do not, and the adverbial use of an adjective stays in its own paradigm with the use recorded on the parse
- See [4-domains/lexical-map.md](4-domains/lexical-map.md), [3-architectural-domains.json](3-architectural-domains.json), and [6-tests-and-build.md](6-tests-and-build.md#lexical-map-domain) for full detail

## Previous Changes (MSB2025 Added, BYZ2018 Retired, Footnote Classifier and Acrostic-Heading Fixes)
...
- **Real YLT1898 Findings Fixed** – Nine outline references across Romans, 1–2 Corinthians, and Revelation's introductory footnotes (e.g. `"Romans 1–11"`) split into their chapter-scoped halves via `auditCrossChapterLinks.ts --fix`, the CLI that later became today's `crossChapterLinks.ts` library module
...
- **New Rule Owner** – `utils/auditCrossChapterLinks.ts`, since renamed to [utils/crossChapterLinks.ts](../../utils/crossChapterLinks.ts), classifies every `bibleLink` target shape (`singleChapter`, `crossChapterRange`, `wholeChapterRange`, `mergedTarget`, `unparsed`) and splits a genuine cross-chapter range into two chapter-scoped links joined by an en dash
- **New Corpus Sweep & CLI** – [utils/crossChapterLinks.ts](../../utils/crossChapterLinks.ts) audits every version (dry-run by default) and writes fixes only when run with `--fix`; exits non-zero on any unsplit finding so it can gate CI like `validate.ts`
...
| [lexical-map.md](../documentation/EGP-Graphai/lexical-map.md)      | Roots, inflected forms, parses, and index crosswalks    |
...
| [4-domains/lexical-map.md](4-domains/lexical-map.md)       | Roots, inflected forms, parses, and index crosswalks |
