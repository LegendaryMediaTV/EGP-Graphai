...
- **Covered scenarios:**
  ...
  - Unresolvable-target judgment (`findUnresolvableTarget`): a target resolving cleanly returns `null`; an unresolvable one names `book-not-in-canon`, `chapter-not-carried`, or `verse-not-carried`. `FAKE_A`'s Mark 9 deliberately omits verse 7 — a real gap mid-chapter, the same shape as ASV1901's Mark 9:44/46 variant case, distinct from a verse past the chapter's highest
...
- **Existing tests** – `utils/__tests__/auditNodes.test.ts` (167 tests, the largest test suite in the repo)
...
- **The four convention specs pin named examples, not corpus totals.** All four used to sweep WEBUS2020's raw source and assert aggregate counts; each now checks the construct against real, named occurrences instead:
  ...
  - `embeddedReferenceConventions.test.ts` — a fully-qualified reference sitting inside ordinary footnote prose becomes a `bibleLink` on the strength of naming its own book and verse, with no `\x` marker and no "See "/"Compare " cue word required. Each case passes a verbatim footnote body to `buildFootnoteContent()`: positives (Matthew 27:35's "and"-joined pair, Proverbs 31:10-31's self-referential acrostic note, Mark 16:9-20, John 8:11's dash-joined pair, Daniel-Greek 3:24, 1 Esdras's three body-initial references) alongside the negatives that must never link (a chapter-only mention even behind a cue word, Esther-Greek 8:13's malformed "Luke 22. 25.", and the three "Psalm NN is an acrostic poem" notes, which name a chapter but no verse)
