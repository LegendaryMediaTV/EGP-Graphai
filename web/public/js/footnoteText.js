 ...
 * @param {boolean} [transliterate] - Read each node's `transliteration` in
 *   place of its `text`, matching what the reader is showing while the
 *   transliteration setting is on. A node carrying none still reads as its own
 *   text, so a tooltip never comes back emptier than the text it describes.
 * @returns {string} The joined display text, or `""` if none can be derived
 */
function getFootnoteText(content, abbreviations, transliterate) {
  if (typeof content === "string") return content;
  if (Array.isArray(content)) {
    return content
      .map((n) => getFootnoteText(n, abbreviations, transliterate))
      .join("");
  }
  if (content && typeof content === "object") {
    if (content.bibleLink)
      return (
        getFootnoteText(content.content, abbreviations, transliterate) ||
        content.bibleLink
      );
    if (content.abbr) {
      const entry = abbreviations && abbreviations.get(content.abbr);
      return entry
        ? getFootnoteText(entry.name, abbreviations, transliterate)
        : content.abbr;
    }
    if (content.text)
      return transliterate && content.transliteration != null
