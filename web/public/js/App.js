...
function App() {
  ...
  const [bookRegistry, setBookRegistry] = useState([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/books")
      .then((res) => res.json())
      .then(setBookRegistry)
      .catch((err) => console.error("Failed to load book registry:", err));
  }, []);
  ...
  const handleBibleLink = (reference) => {
    if (!reference || typeof reference !== "string") return;
    // Match "<book> <chapter>:<verse>" — book is everything before the trailing C:V
    const match = reference.match(/^(.+?)\s+(\d+):(\d+)/);
    if (!match) {
      console.warn(`Could not parse bibleLink reference: "${reference}"`);
      return;
    }
    ...
    if (!book) {
      console.warn(`Unknown book in bibleLink: "${bookName}"`);
      return;
    }
    ...
    setSelectedChapter(Number(chapterStr));
    setActiveFootnote(null);

    // Scroll to the target verse once it has rendered
    const targetVerse = Number(verseStr);
    requestAnimationFrame(() => {
      const tryScroll = () => {
        ...
        if (el) {
          ...
          el.classList.add("bible-link-flash");
          setTimeout(() => el.classList.remove("bible-link-flash"), 1500);
        } else {
          const container = document.getElementById("content-scroll-container");
          if (container) container.scrollTo({ top: 0, behavior: "smooth" });
        }
      };
      // Wait a frame so React has rendered the new chapter
